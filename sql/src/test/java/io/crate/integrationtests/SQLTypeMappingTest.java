...
//                   is((DataType)new ArrayType(new ArrayType(new ArrayType(IntegerType.INSTANCE)))));
    ...
    @Test
    public void testOneLevelNestedObjectInArray() throws Exception {
        execute("create table assets (\n" +
                "    categories array(object (dynamic) as (\n" +
                "       id int)))");
        execute("insert into assets(categories) values ([{id=10}, {id=20}])");
        ensureYellow();
        execute("refresh table assets");

        refresh();
        waitNoPendingTasksOnAll();
        SQLResponse response = execute("select categories[1]['id'] from assets");
        assertEquals(response.rowCount(), 1L);
    }
}
